...
**Editorial note added Session 28.5:** Session 28.5 was an itinerary-planner
polish arc (frontend only, no VOIP/backend/schema change). The Itinerary
Feature section and Key Technical Gotchas below were updated in place to
...
- Claude returns places as JSON (overproduced ~30% for optional suggestions; capped at 30 total — Session 28.5)
...
- Build call **auto-continues** if truncated (long trips) — see Claude API calls below
...
## Session 28.5 — Itinerary Planner Adjustments (July 28, 2026)

Adjustment/polish arc on the AI itinerary planner, done between the VOIP work (Session 28) and the VOIP number-inventory UI (Session 29). **Frontend only — no backend, schema, or VOIP change.** Files touched: `src/pages/Itinerary.js`, `src/pages/Itinerary.module.css`, `src/components/PlacePicker.js`, `src/components/PlacePicker.module.css`. Design decisions recorded separately in `decisions-itinerary-planner.md`.

**1. Opt-in place selection (all unticked)**
- PlacePicker starts with an empty selection Set — neither core nor optional places are pre-ticked.
...
- Each card gets a "More details ▾" expander (styles: detailToggle/detailBox/detailRow). Defensive: no toggle if a place lacks these fields (older saved data).
...
- Manual regenerate: moving a stop marks planDirty → amber banner + "↻ Regenerate plan". No auto-Claude-call per move.
- buildItineraryFromPlaces() extracted as shared builder used by both initial build and regenerate.
...
- `.textarea` style added. Cleared on resetAll.
...
- Verified live: 8-day trip generated complete, no truncation.

**Verification discipline applied:** every file JSX-parsed via esbuild, brace-balance checked, diffed against uploads (additive-only confirmed), salvage parser + continuation loop unit-tested.
...
- Itinerary: auto-continue build call so long trips aren't truncated mid-plan
- docs: Session 28.5 context + itinerary planner decisions (4ed6f714)
